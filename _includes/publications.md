...
<div class="pub-row">
  <div class="col-sm-3 abbr" style="position: relative;padding-right: 15px;padding-left: 15px;">
    {% if link.image %} 
    <img src="{{ link.image | prepend: site.baseurl | replace: '//', '/' }}" class="teaser img-fluid z-depth-1" alt="{{ link.title | escape }}" decoding="async" {% if forloop.first %}fetchpriority="high"{% else %}loading="lazy"{% endif %}>
  ...
  <div class="col-sm-9" style="position: relative;padding-right: 15px;padding-left: 20px;">
    ...
    <div class="links">
      ...
      {% endif %}
      {% if link.add %} 
      <strong> <i style="color:#25B625">{{ link.add }}</i></strong>
      {% endif %}
      {% if link.others %} 
      {{ link.others }}
      {% endif %}
